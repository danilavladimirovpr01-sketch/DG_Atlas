...
import {
  ...
  Brush, Zap, LayoutDashboard,
} from 'lucide-react';
import type { SmetaCategory, Upgrade, MarketRow } from '@/lib/data/smeta-mock';
...
export interface SmetaViewData {
  meta: SmetaMeta;
  categories: SmetaCategory[];
  upgrades: Upgrade[];
...
const ICON_MAP = { Building, Layers, Triangle, Sun, Droplets, AppWindow, Brush, Zap, LayoutDashboard };
...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
function DonutChart({
  categories, total, pricePerSqm, highlightId, onSegmentClick, upgradeExtra,
}: {
  categories: SmetaCategory[];
  total: number;
  pricePerSqm: number;
  ...
  const t = total + upgradeExtra;
  let cursor = 0;
  const segments = categories.map((cat) => {
    const deg = (cat.total / t) * 360;
    const start = cursor;
    cursor += deg;
    ...
    <div className="space-y-4">
      <div className="flex justify-center">
        <div className="relative w-48 h-48">
          <div className="absolute inset-0 rounded-full" style={{ background: `conic-gradient(${gradient})`, filter: 'blur(18px)', opacity: 0.18, transform: 'scale(1.12)' }} />
          ...
          <div className="absolute inset-[30px] rounded-full bg-black flex flex-col items-center justify-center">
            {active ? (
              <>
                ...
                <p className="text-[#555] text-[9px] mt-0.5">{Math.round((active.total / t) * 100)}%</p>
              </>
            ) : (
              <>
                <p className="text-white text-xl font-bold leading-tight">{fmtM(t)}</p>
                <p className="text-[#666] text-[10px] mt-0.5">итого</p>
                <p className="text-[#444] text-[9px] mt-0.5">{pricePerSqm.toLocaleString('ru-RU')} ₽/м²</p>
              </>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {segments.map((seg) => {
          const pct = Math.round((seg.total / t) * 100);
          ...
          return (
            <button
              ...
              style={{ background: isActive ? `${seg.color}12` : '#1a1a1a', borderColor: isActive ? `${seg.color}40` : 'rgba(255,255,255,0.06)' }}
            >
...
/* ═══════════════════════════════════════════════════════
   MARKET COMPARISON
   ═══════════════════════════════════════════════════════ */
function MarketComparison({ rows, city }: { rows: MarketRow[]; city: string }) {
  return (
    <div className="rounded-[20px] bg-[#1a1a1a] border border-white/[0.08] px-5 py-5" style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto text-[#555] text-[10px]">{city}</span>
      </div>
      <div className="space-y-4">
        {rows.map((row, i) => {
          const pct = Math.max(4, Math.min(96, ((row.ours - row.min) / (row.max - row.min)) * 100));
          const isLow = pct < 40; const isMid = pct >= 40 && pct <= 65;
          ...
          return (
            <div key={i}>
              ...
              <div className="relative h-1.5 rounded-full overflow-visible" style={{ background: 'linear-gradient(90deg, rgba(76,217,100,0.25) 0%, rgba(255,152,0,0.25) 100%)' }}>
                <div className="absolute w-3 h-3 rounded-full border-2 border-black -top-[3px] transition-all" style={{ left: `${pct}%`, transform: 'translateX(-50%)', background: dotColor, boxShadow: `0 0 6px ${dotColor}80` }} />
              </div>
              <div className="flex justify-between mt-1.5">
                <span className="text-[#444] text-[10px]">{fmt(row.min)}</span>
                <span className="text-[10px] font-medium" style={{ color: dotColor }}>{isMid || isLow ? 'в рынке ✓' : 'выше среднего'}</span>
...
function CategoryDetail({ cat, onAi }: { cat: SmetaCategory; onAi: (text: string) => void }) {
  const Icon = ICON_MAP[cat.iconName as IconName];
...
/* ═══════════════════════════════════════════════════════
   MAIN COMPONENT — принимает данные через пропсы
   ═══════════════════════════════════════════════════════ */
export default function SmetaWarmFrame({ backHref, data }: { backHref: string; data: SmetaViewData }) {
  const { meta, categories, upgrades, marketComparison, total, hiddenTotal } = data;
  ...
  const paidPct = Math.round((meta.paid / totalWithUpgrades) * 100);
  const activeCat = categories.find((c) => c.id === activeCategory) ?? null;

  function navigateTo(id: string | null) {
    setVisible(false);
    setTimeout(() => { setActiveCategory(id); if (id) setHighlightId(id); setVisible(true); }, 160);
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight leading-tight">
              {activeCat ? activeCat.label : 'Смета этапа'}
            </h1>
            {!activeCat && <p className="text-[#555] text-[11px] mt-0.5">{meta.object} · {meta.version} · {meta.date}</p>}
          </div>
        </div>
        {!activeCat && hiddenTotal > 0 && (
          <button
            onClick={() => { const hid = categories.find(c => c.isHiddenCost); if (hid) navigateTo(hid.id); }}
          ...
          <>
            ...
            <div className="rounded-[20px] bg-gradient-to-br from-[#1a1a1a] to-[#111] border border-white/[0.08] px-5 py-5" style={{ boxShadow: '0 8px 30px rgba(0,0,0,0.4)' }}>
              <div className="flex items-start justify-between mb-1">
                <div>
                  <p className="text-[#555] text-[11px] uppercase tracking-wider">{meta.object} · {meta.area} м²</p>
                  <p className="text-white text-[28px] font-bold tracking-tight leading-none mt-1">{fmt(upgradeExtra > 0 ? totalWithUpgrades : total)} ₽</p>
                  <p className="text-[#666] text-xs mt-1">{meta.pricePerSqm.toLocaleString('ru-RU')} ₽/м²</p>
                ...
                <div className="text-right shrink-0 ml-4">
                  ...
                  <p className="text-white text-xl font-bold mt-0.5">{fmt(meta.paid)} ₽</p>
                  <p className="text-[#555] text-xs mt-0.5">Остаток: {fmt(totalWithUpgrades - meta.paid)} ₽</p>
                </div>
              </div>
              <div className="mt-4">
            ...
            {hiddenTotal > 0 && (
              <div className="rounded-[20px] bg-[#FF3B30]/[0.08] border border-[#FF3B30]/25 px-5 py-4">
                ...
                <p className="text-white text-[28px] font-bold tracking-tight">{fmt(hiddenTotal)} ₽</p>
                <p className="text-[#999] text-[13px] mt-1.5 leading-relaxed font-light">
                  Позиции, которые большинство подрядчиков не включают в базовую цену.
                </p>
                <button
                  onClick={() => { const hid = categories.find(c => c.isHiddenCost); if (hid) navigateTo(hid.id); }}
                  className="flex items-center gap-1 mt-3 text-[#FF3B30] text-xs font-semibold active:opacity-70"
                >
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ borderColor: cat.isHiddenCost ? 'rgba(255,59,48,0.3)' : 'rgba(255,255,255,0.07)', boxShadow: isActive ? `0 0 0 1.5px ${cat.color}50` : 'none' }}
                    >
                  ...
                  {upgrades.map((u) => {
                    ...
                    return (
                      <div key={u.id} className="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-[#1a1a1a] border transition-all" style={{ borderColor: on ? 'rgba(255,152,0,0.3)' : 'rgba(255,255,255,0.06)' }}>
                        ...
                        <button onClick={() => toggleUpgrade(u.id)} className="w-12 h-6 rounded-full transition-all shrink-0 relative" style={{ background: on ? '#FF9800' : '#2a2a2a', border: on ? '1px solid rgba(255,152,0,0.4)' : '1px solid rgba(255,255,255,0.08)' }}>
            ...
            {/* MARKET COMPARISON */}
            {marketComparison.length > 0 && <MarketComparison rows={marketComparison} city={`${meta.city}, ${meta.date.split(' ').slice(-1)[0]}`} />}
          </>
        )}
      </div>
