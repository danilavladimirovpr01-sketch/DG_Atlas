...
/* ═══════════════════════════════════════════════════════
   MARKET COMPARISON — визуальная шкала
   ═══════════════════════════════════════════════════════ */
function MarketComparison() {
  return (
    <div
      className="rounded-[20px] bg-[#1a1a1a] border border-white/[0.08] px-5 py-5"
...
function CategoryDetail({ cat, onAi }: { cat: SmetaCategory; onAi: (text: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Header card */}
      <div
        className="rounded-[20px] bg-[#1a1a1a] border border-white/[0.08] overflow-hidden"
        style={{ boxShadow: `0 0 40px ${cat.color}15` }}
      >
          ...
          {cat.daysFrom && cat.daysTo && (
            <div className="flex items-center gap-2 mt-4 px-3 py-2 rounded-xl bg-white/[0.04]">
              ...
              <span className="text-[#666] text-xs">
                День {cat.daysFrom}–{cat.daysTo} от старта · ~{cat.daysTo - cat.daysFrom} дней
              </span>
            </div>
          )}
        </div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    ...
                    {item.unitPrice && (
                      <>
                        ...
                        <span className="text-[#555] text-xs">{fmt(item.unitPrice)} ₽/{item.unit}</span>
                      </>
                    )}
                  </div>
                </div>
                {item.amount && (
                  <span className="text-white text-sm font-semibold shrink-0 mr-1">{fmt(item.amount)} ₽</span>
                )}
                <button
                  onClick={() => onAi(item.aiText)}
                  className="w-8 h-8 rounded-full bg-[#FF9800]/10 border border-[#FF9800]/20 flex items-center justify-center shrink-0 active:bg-[#FF9800]/20 transition-colors"
                >
      ...
      {hiddenItems.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-2 px-1">
            ...
            <p className="text-[#FF9800] text-xs font-bold uppercase tracking-wider">
              Скрытые позиции · {hiddenItems.length} шт
            </p>
          </div>
          <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-3">
          {activeCat ? (
            <button
              ...
              className="w-10 h-10 flex items-center justify-center rounded-full bg-[#1a1a1a] border border-white/[0.08]"
            >
              <ArrowLeft className="w-5 h-5 text-white" />
            </button>
          ) : (
            <Link href="/tma" className="w-10 h-10 flex items-center justify-center rounded-full bg-[#1a1a1a] border border-white/[0.08]">
              <ArrowLeft className="w-5 h-5 text-white" />
            </Link>
          )}
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight leading-tight">
              {activeCat ? activeCat.label : 'Интерактивная смета'}
            </h1>
            {!activeCat && (
          ...
          <>
              ...
              <div className="flex items-start justify-between mb-1">
                <div>
                  <p className="text-[#555] text-[11px] uppercase tracking-wider">
                    {SMETA_META.object} · {SMETA_META.area} м²
                  </p>
                  ...
                  <p className="text-[#666] text-xs mt-1">
                    {SMETA_META.pricePerSqm.toLocaleString('ru-RU')} ₽/м² · теплый контур
                  </p>
                  {upgradeExtra > 0 && (
                    <p className="text-[#FF9800] text-xs mt-0.5">+{fmt(upgradeExtra)} ₽ с апгрейдами</p>
                  )}
                </div>
            ...
            <div className="rounded-[20px] bg-[#FF3B30]/[0.08] border border-[#FF3B30]/25 px-5 py-4">
              ...
              <p className="text-[#999] text-[13px] mt-1.5 leading-relaxed font-light">
                Дренаж, ливнёвка и отмостка — в чужих сметах этого нет.
                Поэтому их цена выглядит дешевле на первый взгляд.
              </p>
              <button
                onClick={() => navigateTo('engineering')}
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {!cat.isHiddenCost && hiddenCount > 0 && (
                              <span className="px-1.5 py-0.5 rounded-full bg-[#FF9800]/15 text-[#FF9800] text-[10px] font-semibold">
                                !{hiddenCount}
                              </span>
                            )}
                          </div>
                      ...
                      {cat.isHiddenCost && (
                        <div className="mx-4 mb-3 -mt-1 px-3 py-2 rounded-xl bg-[#FF3B30]/[0.08] border border-[#FF3B30]/15">
                          <p className="text-[#FF9090] text-[11px] leading-relaxed">
                            Конкуренты не включают это в базовую цену.
                          </p>
                        </div>
                      )}
                    </button>
            ...
            <div>
              <div className="flex items-center justify-between mb-2 px-1">
                ...
                {upgradeExtra > 0 && (
                  <span className="text-[#FF9800] text-xs font-semibold">+{fmt(upgradeExtra)} ₽</span>
                )}
              </div>
              <div className="space-y-2">
                {UPGRADES.map((u) => {
                  ...
                  return (
                    <div
                      key={u.id}
                      className="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-[#1a1a1a] border transition-all"
                      style={{ borderColor: on ? 'rgba(255,152,0,0.3)' : 'rgba(255,255,255,0.06)' }}
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-white text-sm font-medium">{u.label}</span>
                          {u.popular && (
                            <span className="px-1.5 py-0.5 rounded-full bg-[#FF9800]/15 text-[#FF9800] text-[10px] font-semibold">топ</span>
                          )}
                        </div>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          {relCat && (
                            <>
                              ...
                              <span className="text-[#444] text-[10px]">{relCat.label}</span>
                            </>
                          )}
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        style={{
                          background: on ? '#FF9800' : '#2a2a2a',
                          border: on ? '1px solid rgba(255,152,0,0.4)' : '1px solid rgba(255,255,255,0.08)',
                        }}
                      >
                        <div
                          className="absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all duration-200"
                          style={{ left: on ? '23px' : '2px' }}
                        />
                      </button>
                    </div>
                  );
